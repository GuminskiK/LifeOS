...
async def create_folder(
    folder_in: FolderCreate, session: db_session, owner: current_active_user
):
    return await folder_crud.create_folder(session, folder_in, owner.id)


@router.get("/", response_model=List[FolderRead])
async def list_folders(session: db_session, owner: current_active_user):
    return await folder_crud.fetch_user_folders(session, owner.id)


@router.get("/{folder_id}", response_model=FolderRead)
async def get_folder(folder_id: int, session: db_session, owner: current_active_user):
...
async def delete_folder(
    folder_id: int, session: db_session, owner: current_active_user
):
...
async def move_folder(
    ...
    return await folder_crud.move_folder(
        session, folder_id, new_parent_folder_id, owner.id
    )
