...
async def create_flash_note(session: db_session, flash_note_in: FlashNoteCreate, owner_id: int):
...
async def fetch_flash_note_by_id(session: db_session, flash_note_id: int, owner_id: int):
    result = await session.exec(select(FlashNote).where(FlashNote.id == flash_note_id, FlashNote.owner_id == owner_id))
...
async def update_flash_note(session: db_session, flash_note_update: FlashNoteUpdate, flash_note_id: int, owner_id: int):
    result = await session.exec(select(FlashNote).where(FlashNote.id == flash_note_id, FlashNote.owner_id == owner_id))
