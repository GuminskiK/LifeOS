...
async def create_flash_card(
    session: db_session, flash_card_in: FlashCardCreate, owner_id: int
):
...
async def fetch_flash_card_by_id(
    session: db_session, flash_card_id: int, owner_id: int
):
    result = await session.exec(
        select(FlashCard).where(
            FlashCard.id == flash_card_id, FlashCard.owner_id == owner_id
        )
    )
...
async def update_flash_card(
    session: db_session,
    flash_card_update: FlashCardUpdate,
    flash_card_id: int,
    owner_id: int,
):
    result = await session.exec(
