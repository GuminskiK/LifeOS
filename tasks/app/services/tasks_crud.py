...
from app.models.Tasks import Task, TaskUpdate
from sqlmodel import select
from app.core.exceptions.exceptions import TaskNotFoundException

async def create_task(session: db_session, task_in: dict, owner_id: int):
    db_task = Task(**task_in, owner_id=owner_id)
    session.add(db_task)
...
async def delete_task(session: db_session, task_id: int, owner_id: int):
    result = await session.exec(select(Task).where(Task.id == task_id, Task.owner_id == owner_id))
    db_task = result.one_or_none()
    if not db_task:
        raise TaskNotFoundException()
