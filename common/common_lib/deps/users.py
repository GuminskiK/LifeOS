...
from common_lib.deps.dbs import db_session
...
class AuthDependency:
    ...
    async def get_current_active_user(
        self, token: Optional[str] = Depends(OAuth2PasswordBearer(tokenUrl="token", auto_error=False))
    ) -> CurrentUserContext:
        ...
        try:
            ...
            return CurrentUserContext(id=user_id, username=username, is_superuser=is_superuser)
            
        except JWTError:
            print(f"Exception! Token was {token}")
            raise credentials_exception

    async def get_current_admin_user(self, current_user: CurrentUserContext = Depends(get_current_active_user)) -> CurrentUserContext:
