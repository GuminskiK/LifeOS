...
export const TasksMain: React.FC = () => {
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    setNewTask({ name: '', type: 'task', xp_reward: 10, currency_reward: 5 });
    setShowModal(false);
    fetchData();
  };
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleCreateTask} className="p-6 space-y-4">
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase tracking-widest block mb-2 px-1">Nazwa</label>
                <input 
                  autoFocus
                  value={newTask.name}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase tracking-widest block mb-2 px-1">Typ</label>
                  <select 
                    value={newTask.type}
                  ...
                  >
                    <option value="task">Zwykłe zadanie</option>
                    <option value="habit">Nawyk</option>
                    <option value="milestone">Kamień milowy</option>
                  </select>
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase tracking-widest block mb-2 px-1">Kategoria</label>
                  <select 
              ...
              <div className="bg-blue-50 p-4 rounded-xl flex items-center gap-4">
                 ...
                 <p className="text-xs text-blue-700 font-medium">Ukończenie tego zadania doda Ci +{newTask.xp_reward} XP do poziomu!</p>
              </div>
              <button className="w-full bg-blue-600 text-white font-bold py-4 rounded-xl shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all active:scale-[0.98] mt-2">
                Dodaj do listy
              </button>
            </form>
          </div>
