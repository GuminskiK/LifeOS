...
def test_create_api_key(client):
    ...
    response = client.post("/apikeys?name=MyKey", headers=headers)
...
def test_delete_api_key(client):
    ...
    created = client.post("/apikeys?name=MyKey", headers=headers)
    key_id = created.json().get("id", 1)

    response = client.delete(f"/apikeys/{key_id}?user_id={u_id}", headers=headers)
...
def test_get_my_keys(client):
    ...
    client.post("/apikeys?name=MyKey", headers=headers)
