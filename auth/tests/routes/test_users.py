...
def test_post_no_username(client):
    ...
    response = client.post("/users", json={"plain_password": "TestPassword1!"})
...
def test_get_all_users(client, override_admin):
    ...
    assert any(
        user["username"] == "TestUser" and user["email"] == "test@example.com"
        for user in response_data
    )
    assert any(
        user["username"] == "TestUser2" and user["email"] == "test2@example.com"
        for user in response_data
    )
...
def test_patch_user_ok(client, override_admin):
    ...
    response = client.patch("/users/1", json={"username": "TestUserPatched"})
...
def test_patch_unknown_field(client, override_admin):
    ...
    response = client.patch(
        "/users/1", json={"username": "TestUserPatched", "unknown": "-------"}
    )
