...
def test_login_success(client):
    ...
    client.post(
        ...
        json={
            ...
            "plain_password": "Auth!Password1",
        },
    )

    # Login
    response = client.post(
        "/auth/token", data={"username": "AuthUser", "password": "Auth!Password1"}
    )
...
def test_login_invalid_credentials(client):
    ...
    response = client.post(
        "/auth/token", data={"username": "AuthUser2", "password": "Wr0ng!Password"}
    )
...
def test_login_with_2fa(client):
    ...
    client.post(
        ...
        json={
            ...
            "plain_password": "P@ssw0rd1",
        },
    )

    # First login to setup 2FA
    login_resp = client.post(
        "/auth/token", data={"username": "TwoFaUser", "password": "P@ssw0rd1"}
    )
...
def test_refresh_token(client):
    ...
    login_resp = client.post(
        "/auth/token", data={"username": "RefUser", "password": "P@ssw0rd1"}
    )
    ...
    refresh_fail = client.post("/auth/refresh", json={"refresh_token": refresh_token})
...
def test_refresh_token_reuse_detection(client):
    ...
    login_resp = client.post(
        "/auth/token", data={"username": "ReuseUser", "password": "P@ssw0rd1"}
    )
    ...
    assert reuse_resp.json()["detail"] in [
...
def test_logout(client):
    ...
    login_resp = client.post(
        "/auth/token", data={"username": "OutUser", "password": "P@ssw0rd1"}
    )
    refresh_token = login_resp.json()["refresh_token"]
...
def test_get_sessions_and_logout_specific(client):
    ...
    delete_resp = client.post(
        f"/auth/logout/{sid2}?user_id=1", headers={"Authorization": f"Bearer {token1}"}
    )
    assert delete_resp.status_code == 200

    # Verify it was deleted
    sessions_resp2 = client.get(
        "/auth/sessions?user_id=1", headers={"Authorization": f"Bearer {token1}"}
    )
...
def test_activate_account(mock_send_message, client):
    ...
    activation_token = create_token(
        user_id, "ActivationTestUser", TokenTypes.ACTIVATE, timedelta(days=1)
    )
...
def test_change_password_flow(mock_send_message, client):
    ...
    forgot_resp = client.post(
        "/auth/forgot_password", json={"email": "pass@example.com"}
    )
    ...
    reset_token = create_token(
        user_id, "PassUser", TokenTypes.CHANGE_PASSWORD, timedelta(minutes=60)
    )

    # 4. Wykorzystanie go na endpoincie zmiany hasla
    new_password = "N3wSuper!Password"
    change_resp = client.patch(
        f"/auth/change_password/{reset_token}", json={"plain_password": new_password}
    )
    ...
    fail_login = client.post(
        "/auth/token", data={"username": "PassUser", "password": "0ldP@ssword"}
    )
    assert fail_login.status_code == 401

    # 6. Weryfikacja (Logowanie Nowym haslem dziala)
    success_login = client.post(
        "/auth/token", data={"username": "PassUser", "password": new_password}
    )
