...
def test_create_access_token():
    encoded_token = create_token(
        1, "TestUser", TokenTypes.ACCESS, timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    )
...
def test_create_refresh_token():
    encoded_token = create_token(
        1, "TestUser", TokenTypes.REFRESH, timedelta(days=REFRESH_TOKEN_EXPIRE_DAYS)
    )
    token = decode_token(encoded_token)
    assert token["sub"] == "TestUser"
    assert token["user_id"] == 1
...
async def test_store_refresh_token():
    ...
    sid = await store_refresh_token(
        redis, jti, user_id, exp, device="iOS", ip="127.0.0.1"
    )
...
async def test_revoke_all_user_sessions():
    ...
    sid1 = await store_refresh_token(
        redis, jti1, user_id, int(_now().timestamp()) + 3600
    )
    sid2 = await store_refresh_token(
        redis, jti2, user_id, int(_now().timestamp()) + 3600
    )
