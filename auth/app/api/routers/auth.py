...
async def post_refresh_token(
    redis: redis_client, refresh_token: str = Body(..., embed=True)
):
...
async def patch_superuser_status(
    session: db_session, user_id: int, admin: current_admin_user
):
...
async def forgot_password(
    ...
    return {
        "message": "Jeśli to konto instnieje, wysłaliśmy instrukcje resetu hasła na wskazany adres e-mail."
    }


@router.patch("/change_password/{password_change_token}")
async def patch_password(
    session: db_session, password_change_token: str, payload: NewPasswordModel
):
